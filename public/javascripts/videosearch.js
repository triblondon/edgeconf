var iframe = document.querySelector('iframe#youTubePlayer');

var videoTopics = {
  'offline': 'Oic22dQMRXQ',
  'testing': 'FY5iiuQRyEE',
  'responsive': 'WhZpKdQcKhU',
  'network': 'fX0j4c_NAFY',
  'input': 'zxuA-wyajhY',
  'privileged': 'ytJKdipILiU',
  'performance': '3-WYu_p5rdU'
};

  var watchVideoDivs = document.querySelectorAll("div.watchVideo");
  for (var i = 0; i != watchVideoDivs.length; ++i){
    var div = watchVideoDivs[i];
    div.onclick = function(){
      var topic = this.dataset['topic'];
      var videoId = videoTopics[topic];
      document.location.href = '#video';
      iframe.src = 'http://www.youtube.com/embed/' + videoId + '?autoplay=1&enablejsapi=1';
    };
  }
...
resultsDiv.css('max-height', Math.max(iframe.offsetHeight-80, 150));
