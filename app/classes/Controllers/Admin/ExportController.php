	...
	public function get() {
		...
		if ($this->routeargs['export'] == 'panels') {
			...
			foreach ($sessions as $session) {
				...
				$panelists = $this->app->db->queryAllRows('SELECT pe.family_name, pe.given_name, pa.role FROM participation pa INNER JOIN people pe ON pa.person_id=pe.id WHERE pa.session_id=%d AND role != %s AND panel_status=%s', $session['id'], 'Delegate', 'Confirmed');
